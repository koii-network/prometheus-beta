...
def test_zero_rotation():
    """Test rotation by zero positions leaves array unchanged"""
    arr = [1, 2, 3, 4, 5]
    result = rotate_array_left(arr, 0)
    assert result == arr
    assert result is not arr  # Ensure a new list is returned

def test_full_length_rotation():
    """Test rotation by full array length returns the same array"""
    arr = [1, 2, 3, 4, 5]
...
def test_invalid_rotation_type():
    """Test raising TypeError for non-integer rotation amount"""
...
def test_negative_rotation():
    """Test raising ValueError for negative rotation amount"""
