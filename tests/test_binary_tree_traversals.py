import pytest
from src.binary_tree_traversals import Node, in_order_traversal, pre_order_traversal, post_order_traversal
...
def test_skewed_tree():
    """Test traversals on a skewed tree (only left children)"""
    #   5
    #  /
    # 4
    #/
    ...
    # In-order: Left -> Root -> Right
    assert in_order_traversal(root) == [1, 2, 3, 4, 5]

    # Pre-order: Root -> Left -> Right
    assert pre_order_traversal(root) == [5, 4, 3, 2, 1]
